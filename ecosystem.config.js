module.exports = {
  ...
  deploy: {
    production: {
      ...
      'post-deploy': 'npm install && pm2 reload ecosystem.config.js --env production', // 顺序执行这些指令，最后 pm2 startOrReload ecosystem.config.js 会通过 pm2 启动 vue3-admin-server.js
      env: {
        NODE_ENV: 'production'
      }
    }
